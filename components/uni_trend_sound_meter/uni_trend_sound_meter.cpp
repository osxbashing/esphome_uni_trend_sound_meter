...
void UnitTrendSoundMeter::gattc_event_handler(
  ...
  switch (event) {
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT: {
      ESP_LOGI(TAG, "[%s] Service discovery completed", this->get_name().c_str());
    ...
    case ESP_GATTC_NOTIFY_EVT: {
      if (param->notify.conn_id != this->parent()->get_conn_id() ||
          param->notify.handle != this->output_handle_)
        break;

      ESP_LOGV(TAG, "[%s] GATT Notification: handle=0x%x, value_length=%d",
               this->get_name().c_str(),
               param->notify.handle,
...
void UnitTrendSoundMeter::update() {
  ...
  if (this->input_handle_ == 0) {
    ESP_LOGW(TAG, "[%s] Cannot poll, no input characteristic", this->get_name().c_str());
    return;
  }
  ...
  if (status != ESP_OK) {
    this->status_set_warning();
    ESP_LOGW(TAG, "[%s] Error sending write request, status=%d", this->get_name().c_str(), status);
  }
}
